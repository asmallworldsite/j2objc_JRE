...
//  source: /Users/tball/src/j2objc/guava/android/build_result/java/com/google/common/hash/HashFunction.java
//
...
/*!
 @brief A hash function is a collision-averse pure function that maps an arbitrary block of data to a
 ...
 <code>Hasher</code>), but this is merely a convenience; these are always translated into raw
        byte sequences under the covers.   
 <li><b>hash code:</b> each hash function always yields hash codes of the same fixed bit length
        (given by <code>bits</code>). For example, <code>Hashing.sha1</code> produces a 160-bit number,
        while <code>Hashing.murmur3_32()</code> yields only 32 bits. Because a <code>long</code> value is
        clearly insufficient to hold all hash code values, this API represents a hash code as an
        instance of <code>HashCode</code>.
    <li><b>pure function:</b> the value produced must depend only on the input bytes, in the order
        they appear. Input data is never modified. <code>HashFunction</code> instances should always be
 ...
 <i>any</i> eight bits could be consistently used.
    <li><b>cryptographic:</b> certain hash functions such as <code>Hashing.sha512</code> are designed to
 ...
 <p>The primary way to provide the data that your hash function should act on is via a <code>Hasher</code>
 . Obtain a new hasher from the hash function using <code>newHasher</code>, "push" the relevant
  data into it using methods like <code>Hasher.putBytes(byte[])</code>, and finally ask for the <code>HashCode</code>
  when finished using <code>Hasher.hash</code>. (See an example of
  this.) 
 <p>If all you want to hash is a single byte array, string or <code>long</code> value, there are
  convenient shortcut methods defined directly on <code>HashFunction</code> to make this easier. 
 <p>Hasher accepts primitive data types, but can also accept any Object of type <code>T</code> provided
  that you implement a <code>Funnel</code><code><T></code> to specify how to "feed" data from that object
 ...
 <i>little-endian</i> order. That is, hashing the byte array <code>{0x01, 0x02, 0x03, 0x04}</code> is
  ...
  methods such as <code>Integer.reverseBytes</code> and <code>Ints.toByteArray</code> will help. 
 <h3>Relationship to <code>Object.hashCode</code></h3>
 ...
 <p><code>Object.hashCode</code> implementations tend to be very fast, but have weak collision
  ...
  however, <code>Object.hashCode</code> almost always falls short -- hence this library.
...
/*!
 @brief Begins a new hash code computation by returning an initialized, stateful <code>Hasher</code>
...
/*!
 @brief Begins a new hash code computation as <code>newHasher()</code>, but provides a hint of the expected
...
/*!
 @brief Shortcut for <code>newHasher().putInt(input).hash()</code>; returns the hash code for the given 
...
/*!
 @brief Shortcut for <code>newHasher().putLong(input).hash()</code>; returns the hash code for the given 
...
/*!
 @brief Shortcut for <code>newHasher().putBytes(input).hash()</code>.The implementation <i>might</i>
  perform better than its longhand equivalent, but should not perform worse.
 */
- (ComGoogleCommonHashHashCode *)hashBytesWithByteArray:(IOSByteArray * __nonnull)input;

/*!
 @brief Shortcut for <code>newHasher().putBytes(input, off, len).hash()</code>.The implementation 
...
/*!
 @brief Shortcut for <code>newHasher().putUnencodedChars(input).hash()</code>.The implementation 
 <i>might</i> perform better than its longhand equivalent, but should not perform worse.
 Note
 ...
 <code>hashString</code>, usually with a charset of UTF-8. For other use cases, use <code>hashUnencodedChars</code>
 .
...
/*!
 @brief Shortcut for <code>newHasher().putString(input, charset).hash()</code>.Characters are encoded using
  the given <code>Charset</code>.
 The implementation <i>might</i> perform better than its longhand
 ...
 <p><b>Warning:</b> This method, which reencodes the input before hashing it, is useful only for
  cross-language compatibility. For other use cases, prefer <code>hashUnencodedChars</code>, which is
...
/*!
 @brief Shortcut for <code>newHasher().putObject(instance, funnel).hash()</code>.The implementation 
 <i>might</i> perform better than its longhand equivalent, but should not perform worse.
 @since 14.0
 */
...
/*!
 @brief Returns the number of bits (a multiple of 32) that each hash code produced by this hash
  function has.
 */
- (jint)bits;
