...
/*!
 @brief Checks the whole hash table for poor hash distribution.Takes O(n) in the worst case, O(n / log
  n) on average.
 ...
 <p>This method may return <code>true</code> up to <code>HASH_FLOODING_FPP</code> of the time even on
  truly random input. 
 <p>If this method returns false, there are definitely no runs of length at least <code>maxRunBeforeFallback(hashTable.length)</code>
  nonnull elements. If there are no runs of length at
  least <code>maxRunBeforeFallback(hashTable.length) / 2</code> nonnull elements, this method
  definitely returns false. In between those constraints, the result of this method is undefined,
  subject to the above <code>HASH_FLOODING_FPP</code> constraint.
 */
+ (jboolean)hashFloodingDetectedWithNSObjectArray:(IOSObjectArray * __nonnull)hashTable;
...
#define ComGoogleCommonCollectImmutableSet_MAX_RUN_MULTIPLIER 13
...
- (ComGoogleCommonCollectImmutableSet_Builder *)combineWithComGoogleCommonCollectImmutableSet_Builder:(ComGoogleCommonCollectImmutableSet_Builder * __nonnull)other;
