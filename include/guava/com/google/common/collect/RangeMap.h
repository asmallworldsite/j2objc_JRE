 ...
 */
- (id)getWithJavaLangComparable:(id<JavaLangComparable> __nonnull)key;
...
/*!
 @brief Merges a value into the map over a range by applying a remapping function.
 <p>If any parts of the range are already present in this range map, those parts are mapped to
  ...
  this range map are mapped to the specified value, unless the value is <code>null</code>.
 ...
 <p>For example, if <code>rangeMap</code> had one entry <code>[1, 5] => 3</code> then <code>rangeMap.merge(Range.closed(0,2), 3, Math::max)</code>
  could yield a range map with the entries 
 <code>[0, 1) => 3, [1, 2] => 3, (2, 5] => 3</code>.
 @since 28.1
 */
- (void)mergeWithComGoogleCommonCollectRange:(ComGoogleCommonCollectRange * __nonnull)range
                                      withId:(id __nullable)value
              withJavaUtilFunctionBiFunction:(id<JavaUtilFunctionBiFunction> __nonnull)remappingFunction;
