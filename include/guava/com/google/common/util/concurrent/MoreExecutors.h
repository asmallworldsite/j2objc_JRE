...
/*!
 @brief Factory and utility methods for <code>java.util.concurrent.Executor</code>, <code>ExecutorService</code>,
  and <code>java.util.concurrent.ThreadFactory</code>.
...
+ (void)addDelayedShutdownHookWithJavaUtilConcurrentExecutorService:(id<JavaUtilConcurrentExecutorService> __nonnull)service
                                               withJavaTimeDuration:(JavaTimeDuration * __nonnull)terminationTimeout;
...
/*!
 ...
 , as in <code>ThreadPoolExecutor.CallerRunsPolicy</code>.
 <p>This executor is appropriate for tasks that are lightweight and not deeply chained.
  Inappropriate <code>directExecutor</code> usage can cause problems, and these problems can be
  difficult to reproduce because they depend on timing. For example: 
 <ul>
    <li>A call like <code>future.transform(function, directExecutor())</code> may execute the function
        immediately in the thread that is calling <code>transform</code>. (This specific case happens
        if the future is already completed.) If <code>transform</code> call was made from a UI thread
 ...
 <li>If many tasks will be triggered by the same event, one heavyweight task may delay other
        tasks -- even tasks that are not themselves <code>directExecutor</code> tasks.
    <li>If many such tasks are chained together (such as with <code>future.transform(...).transform(...).transform(...)....</code>
 ), they may overflow the stack.
        (In simple cases, callers can avoid this by registering all tasks with the same <code>MoreExecutors.newSequentialExecutor</code>
  wrapper around <code>directExecutor()</code>. More
        complex cases may require using thread pools or making deeper changes.) 
 </ul>
  Additionally, beware of executing tasks with <code>directExecutor</code> while holding a lock. Since
...
/*!
 ...
 , as in <code>ThreadPoolExecutor.CallerRunsPolicy</code>.This applies both to
  individually submitted tasks and to collections of tasks submitted via <code>invokeAll</code> or 
 <code>invokeAny</code>.
 In the latter case, tasks will run serially on the calling thread. Tasks are
  run to completion before a <code>Future</code> is returned to the caller (unless the executor has
  been shutdown). 
...
/*!
 ...
 <p>When running on AppEngine with access to <a href="https://cloud.google.com/appengine/docs/standard/java/javadoc/">
 AppEngine legacy
  APIs</a>, this method returns <code>ThreadManager.currentRequestThreadFactory()</code>. Otherwise,
  it returns <code>Executors.defaultThreadFactory()</code>.
 @since 14.0
 */
...
FOUNDATION_EXPORT id<JavaUtilConcurrentExecutorService> ComGoogleCommonUtilConcurrentMoreExecutors_getExitingExecutorServiceWithJavaUtilConcurrentThreadPoolExecutor_withJavaTimeDuration_(JavaUtilConcurrentThreadPoolExecutor *executor, JavaTimeDuration *terminationTimeout);
...
FOUNDATION_EXPORT id<JavaUtilConcurrentScheduledExecutorService> ComGoogleCommonUtilConcurrentMoreExecutors_getExitingScheduledExecutorServiceWithJavaUtilConcurrentScheduledThreadPoolExecutor_withJavaTimeDuration_(JavaUtilConcurrentScheduledThreadPoolExecutor *executor, JavaTimeDuration *terminationTimeout);
...
FOUNDATION_EXPORT void ComGoogleCommonUtilConcurrentMoreExecutors_addDelayedShutdownHookWithJavaUtilConcurrentExecutorService_withJavaTimeDuration_(id<JavaUtilConcurrentExecutorService> service, JavaTimeDuration *terminationTimeout);
...
FOUNDATION_EXPORT id ComGoogleCommonUtilConcurrentMoreExecutors_invokeAnyImplWithComGoogleCommonUtilConcurrentListeningExecutorService_withJavaUtilCollection_withBoolean_withJavaTimeDuration_(id<ComGoogleCommonUtilConcurrentListeningExecutorService> executorService, id<JavaUtilCollection> tasks, jboolean timed, JavaTimeDuration *timeout);
...
FOUNDATION_EXPORT jboolean ComGoogleCommonUtilConcurrentMoreExecutors_shutdownAndAwaitTerminationWithJavaUtilConcurrentExecutorService_withJavaTimeDuration_(id<JavaUtilConcurrentExecutorService> service, JavaTimeDuration *timeout);
