...
//  source: /Users/tball/src/j2objc/guava/build_result/java/com/google/common/util/concurrent/AbstractService.java
//
...
/*!
 @since 28.0
 */
- (void)awaitRunningWithJavaTimeDuration:(JavaTimeDuration * __nonnull)timeout;
...
/*!
 @since 28.0
 */
- (void)awaitTerminatedWithJavaTimeDuration:(JavaTimeDuration * __nonnull)timeout;
...
/*!
 @brief This method is called by <code>stopAsync</code> when the service is still starting (i.e.
 <code>startAsync</code>
  has been called but <code>notifyStarted</code> has not). Subclasses can override the
  method to cancel pending work and then call <code>notifyStopped</code> to stop the service. 
 <p>This method should return promptly; prefer to do work on a different thread where it is
  convenient. It is invoked exactly once on service shutdown, even when <code>stopAsync</code> is
  called multiple times. 
 <p>When this method is called <code>state()</code> will return <code>State.STOPPING</code>, which is the
  external state observable by the caller of <code>stopAsync</code>.
 @since 27.0
 */
- (void)doCancelStart;
...
/*!
 ...
 <p>This method should return promptly; prefer to do work on a different thread where it is
  convenient. It is invoked exactly once on service shutdown, even when <code>stopAsync</code> is
  called multiple times. 
 <p>If <code>stopAsync</code> is called on a <code>State.STARTING</code> service, this method is not
  invoked immediately. Instead, it will be deferred until after the service is <code>State.RUNNING</code>
 . Services that need to cancel startup work can override <code>doCancelStart</code>.
 */
- (void)doStop;
...
/*!
 @brief Implementing classes should invoke this method once their service has stopped.It will cause
  the service to transition from <code>State.STARTING</code> or <code>State.STOPPING</code> to <code>State.TERMINATED</code>
 .
 @throw IllegalStateExceptionif the service is not one of <code>State.STOPPING</code>, <code>State.STARTING</code>
 , or <code>State.RUNNING</code>.
 */
- (void)notifyStopped;
